...
@pytest.mark.parametrize(
    ...
    [
        ("10002000300040005000", ValueError),
        ("100020003000", ValueError),
        (1000200030004000, TypeError),
        ("", ValueError),
    ],
)
def test_get_mask_card_number_raise(card_number: str, expected_result: type[BaseException]) -> None:
    """проверка на ошибки"""
    with pytest.raises(expected_result):
        get_mask_card_number(card_number)


def test_get_mask_check() -> None:
    """проверка стандартного ввода"""
    assert get_mask_check("10002000300040005000") == "**5000"


@pytest.mark.parametrize(
    "check_number, expected_result",
    [
        ("", ValueError),
...
def test_get_mask_check_number_raise(check_number: str, expected_result: type[BaseException]) -> None:
    """проверка на ошибки"""
    with pytest.raises(expected_result):
